...
# Test that a single step of a stepped upscale completes successfully.
def test_scaler_down_to_upscale_stepped(monkeypatch):
    api = MagicMock()
    monkeypatch.setattr(
        "kube_downscaler.scaler.helper.get_kube_api", MagicMock(return_value=api)
    )
    ...
    assert json.loads(api.patch.call_args[1]["data"])["metadata"]["annotations"][
        ORIGINAL_REPLICAS_ANNOTATION
    ]


# Test that the last step of a stepped upscale completes successfully.
def test_scaler_down_to_upscale_stepped_complete(monkeypatch):
    api = MagicMock()
    monkeypatch.setattr(
        "kube_downscaler.scaler.helper.get_kube_api", MagicMock(return_value=api)
    )
